...
std::string map_service = "/slam_toolbox/dynamic_map";
...
class GlobalRRT : public rclcpp::Node
{
  public:
    ...
    : Node("global_rrt"), are_boundary_points_receieved(false)
    {
        ...
        this->declare_parameter<std::string>("map_service", map_service);
        this->get_parameter("map_topic", map_topic);
        this->get_parameter("map_service", map_service);

        map_subscription_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        map_topic, 100, std::bind(&GlobalRRT::map_topic_callback, this, _1));
        map_client_ = this->create_client<GetMap>(map_service);
  ...
  private:
    ...
    {
        mapData = msg;
        init_map_x = std::max<double>(mapData.info.width * mapData.info.resolution, eta);
        init_map_y = std::max<double>(mapData.info.height * mapData.info.resolution, eta);
        Xstartx = mapData.info.origin.position.x + init_map_x * 0.5;
        Xstarty = mapData.info.origin.position.y + init_map_y * 0.5;
        are_boundary_points_receieved = true;
    }

    void request_map()
    {
        if (map_request_in_flight_)
        {
            return;
        }
        ...
        if (!map_client_->service_is_ready())
        {
            RCLCPP_INFO_THROTTLE(
                ...
                "Map service %s is not ready yet", map_service.c_str());
            return;
        }

        auto request = std::make_shared<GetMap::Request>();
        map_request_in_flight_ = true;
        map_client_->async_send_request(
            request,
            [this](rclcpp::Client<GetMap>::SharedFuture future)
            {
                map_request_in_flight_ = false;
                try
                {
                    ...
                    if (response->map.data.empty())
                    {
                        RCLCPP_WARN_THROTTLE(
                            this->get_logger(), *this->get_clock(), 2000,
                            "Map service returned an empty map");
                        return;
                    }

                    update_map(response->map);
                    RCLCPP_INFO_THROTTLE(
                        this->get_logger(), *this->get_clock(), 5000,
                        "Loaded map from service %s", map_service.c_str());
                }
                catch (const std::exception & ex)
                {
                    RCLCPP_WARN_THROTTLE(
                        this->get_logger(), *this->get_clock(), 2000,
                        "Map service request failed: %s", ex.what());
                }
            });
    }

    std::vector<double> find_seed_point()
    {
        std::vector<double> map_center = {
            mapData.info.origin.position.x + (mapData.info.width * mapData.info.resolution * 0.5),
            mapData.info.origin.position.y + (mapData.info.height * mapData.info.resolution * 0.5)};
        ...
        for (size_t i = 0; i < mapData.data.size(); ++i)
        {
            ...
            std::vector<double> candidate = point_of_index(mapData, static_cast<int>(i));
            double distance = norm(candidate, map_center);
    ...
    void timer_callback(){
        if(mapData.data.empty()){
            request_map();
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "Waiting for the map");
            return;
        }
    ...
    bool map_request_in_flight_ = false;
};
