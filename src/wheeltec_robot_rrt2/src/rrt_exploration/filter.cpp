...
class Filter : public rclcpp::Node
{
  public:
    ...
    : Node("filter")
    {
        ...
        this->declare_parameter<std::string>("global_costmap_service", "/global_costmap/get_costmap");
        ...
        this->get_parameter("global_costmap_service", global_costmap_service);
        ...
        global_costmap_client_ = this->create_client<GetCostmap>(global_costmap_service);
  ...
  private:
    ...
    {
        global_map = *msg;
        global_costmap_request_in_flight_ = false;
    }
    ...
    nav_msgs::msg::OccupancyGrid to_occupancy_grid(const nav2_msgs::msg::Costmap & costmap_msg) const
    {
        nav_msgs::msg::OccupancyGrid occupancy_grid;
        occupancy_grid.header = costmap_msg.header;
        occupancy_grid.info.resolution = costmap_msg.metadata.resolution;
        occupancy_grid.info.width = costmap_msg.metadata.size_x;
        occupancy_grid.info.height = costmap_msg.metadata.size_y;
        occupancy_grid.info.origin = costmap_msg.metadata.origin;
        occupancy_grid.data.reserve(costmap_msg.data.size());

        for (auto cell : costmap_msg.data) {
            occupancy_grid.data.push_back(static_cast<int8_t>(cell));
        }

        return occupancy_grid;
    }

    void request_global_costmap()
    {
        if (global_costmap_request_in_flight_) {
            return;
        }
        ...
        if (!global_costmap_client_->service_is_ready()) {
            RCLCPP_INFO_THROTTLE(
                ...
                "Waiting for global costmap service: %s", global_costmap_service.c_str());
            return;
        }

        auto request = std::make_shared<GetCostmap::Request>();
        global_costmap_request_in_flight_ = true;
        global_costmap_client_->async_send_request(
            request,
            [this](rclcpp::Client<GetCostmap>::SharedFuture future) {
                global_costmap_request_in_flight_ = false;
                try {
                    const auto response = future.get();
                    global_map = to_occupancy_grid(response->map);
                    if (global_map.data.empty()) {
                        RCLCPP_WARN_THROTTLE(
                            this->get_logger(), *this->get_clock(), 5000,
                            "Global costmap service responded, but the returned map is empty");
                    } else {
                        RCLCPP_INFO_THROTTLE(
                            this->get_logger(), *this->get_clock(), 5000,
                            "Fetched global costmap from service: %s", global_costmap_service.c_str());
                    }
                } catch (const std::exception & ex) {
                    RCLCPP_WARN_THROTTLE(
                        this->get_logger(), *this->get_clock(), 5000,
                        "Failed to request global costmap from %s: %s",
                        global_costmap_service.c_str(), ex.what());
                }
            });
    }
    ...
    void timer_callback(){
        if(mapData.data.empty()){
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "Waiting for the map");
            return;
        }
        if(global_map.data.empty()){
            request_global_costmap();
            if (global_map.data.empty()) {
                RCLCPP_INFO_THROTTLE(
                    this->get_logger(), *this->get_clock(), 5000,
                    "Waiting for the global map topic or service response");
                return;
            }
        }
        if(frontiers.empty()){
    ...
    std::string map_topic, goals_topic, global_costmap_topic, global_costmap_service;
    int threshold; 
    double info_radius;
    ...
    rclcpp::Client<GetCostmap>::SharedPtr global_costmap_client_;
    ...
    bool global_costmap_request_in_flight_ = false;
};
