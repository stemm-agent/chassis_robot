...
class radarFollower(Node):

	def __init__(self):
		super().__init__('radarfollower')
		...
		self.radarfwflagPublisher = self.create_publisher(Int8,'/radar_follow_flag',qos)
		...
		self.trackerInfoSubscriber = self.create_subscription(
		    ...
		    qos)
		self.declare_parameter('targetDist',1.2)
		...
		self.PID_controller = simplePID([0, targetDist], [2.6, 1.0], [0, 0], [0.00,0.000])
	...
	def publish_flag(self):
		...
		self.radarfwflagPublisher.publish(radar_follow_flag)
		self.get_logger().info('radar_follow_flag: {}'.format(radar_follow_flag))
...
def main(args=None):
    ...
    radarfollower = radarFollower()
    try:
        rclpy.spin(radarfollower)
    finally:
        radarfollower.destroy_node()
        rclpy.shutdown()
