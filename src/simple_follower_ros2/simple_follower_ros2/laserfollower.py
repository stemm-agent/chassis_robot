...
class LaserFollower(Node):
	...
	def __init__(self):
		...
		self.declare_parameter('I',[0.00,0.00])
		self.declare_parameter('D',[0.03 ,0.005])
		self.declare_parameter('~targetDist',[0.0,0.6])
		...
		self.max_speed = self.declare_parameter('~maxSpeed',0.5)
		...
		self.get_logger().info('simplepid_parameter: target:{} P:{} I:{} D:{}'.format(targetDist,P,I,D))
		self.PID_controller = simplePID(targetDist,P,I,D)
	...
	def positionUpdateCallback(self, position):
		...
		angle_error=abs(abs(angle_x)-3.14)
		linear_error=abs(distance-0.8)
		if distance>0.3 and angle_error > 0.05 and linear_error > 0.05:
			velocity.linear.x = linearSpeed
			velocity.angular.z = -angularSpeed
			#self.get_logger().info('distance: {}, angle_error: {},linear_error:{}'.format(distance, angle_error,linear_error))
		else:
			velocity.linear.x = 0.0
			velocity.angular.z = 0.0
			#self.get_logger().info('linear_error: {}, angle_error: {}'.format(linear_error, angle_error))
			#self.get_logger().info('distance: {}, angle_error: {},linear_error:{}'.format(distance, angle_error,linear_error))
