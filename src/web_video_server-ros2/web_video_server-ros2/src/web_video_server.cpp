...
static std::vector<std::pair<std::string, std::string>> get_ipv4_addresses()
{
  ...
  for (struct ifaddrs *ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    if (inet_ntop(AF_INET, &addr->sin_addr, ip, sizeof(ip)) != nullptr) {
      addresses.emplace_back(ifa->ifa_name, ip);
    }
  }
...
{
  // Declare parameters on the serving node so launch and CLI overrides apply normally in ROS 2.
  port_ = nh_->declare_parameter<int>("port", 8090);
  __verbose = nh_->declare_parameter<bool>("verbose", true);
  address_ = nh_->declare_parameter<std::string>("address", "0.0.0.0");

  int server_threads = nh_->declare_parameter<int>("server_threads", 1);
  ros_threads_ = nh_->declare_parameter<int>("ros_threads", 2);
  publish_rate_ = nh_->declare_parameter<double>("publish_rate", -1.0);
  __default_stream_type = nh_->declare_parameter<std::string>("default_stream_type", "mjpeg");
...
void WebVideoServer::spin()
{
  ...
  if (address_ == "0.0.0.0") {
    auto addresses = get_ipv4_addresses();
    for (const auto &entry : addresses) {
      RCLCPP_INFO(nh_->get_logger(), "Accessible via %s: http://%s:%d/", entry.first.c_str(), entry.second.c_str(), port_);
    }
  }
