...
bool Wheeltec_Mic::Send_Serial_Data(ServicePacket &pkg)
{
  ...
  {
    RCLCPP_WARN(this->get_logger(), "Serial port is unavailable; command was not sent");
    return false;
  }
  ...
  const std::string message = MakeMsgPacket(pkg.sid, MsgType::CONTROL, section);
  try
  {
    MicArr_Serial.write(message);
  }
