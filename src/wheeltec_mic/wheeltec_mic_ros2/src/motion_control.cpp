...
void play_audio_feedback_with_flag(const char* audio_file) {
    ...
    if (command) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("motion_control"), 
...
返回  值：无
**************************************************************************/
void Motion::send_goal(std::vector<float> msg)
{
...
{
	angle = msg->data;
	awake_angle_valid = true;
}

void Motion::mapping_motion_blocked_Callback(
	const std_msgs::msg::Bool::SharedPtr msg)
{
	...
	if (blocked && !mapping_motion_blocked_)
	{
		...
		RCLCPP_WARN(this->get_logger(),
			"Cartographer mapping gate active; released voice cmd_vel ownership");
		return;
	}
...
void Motion::motion_msg_Callback(const wheeltec_mic_msg::msg::MotionControl::SharedPtr msg)
{
	...
	{
		if (msg->cmd_vel_flag && msg->linear_x == 0.0 && msg->angular_z == 0.0)
		{
	...
	if (msg->follow_flag && !awake_angle_valid && !follow_flag)
	{
	        RCLCPP_WARN(this->get_logger(),
	                "Ignoring follow command because no valid awake angle has been received");
	        follow_flag = 0;
	}
	else
	{
...
功能: 底盘运动控制
*********************************************************/
void Motion::timer_callback()
{
    static int temp_cout = 0;
	if (mapping_motion_blocked_)
	{
		temp_cout = 0;
		return;
    ...
    // 处理导航目标发送和状态检查
    if (is_following_goal && !goal_sent){
        send_goal(point);
        goal_sent = true;
        RCLCPP_INFO(this->get_logger(), "Navigation goal sent");
    }

    if (follow_flag){
        awake_angle_valid = false;
        if (if_akm) {
    ...
    // akm模型的导航完成检查
    if (is_following_goal && goal_reached_flag){
        is_following_goal = false;
        follow_flag = 0;
        turn_fin_flag = 1;
        ...
        goal_reached_flag = 0;
        RCLCPP_INFO(this->get_logger(), "Navigation completed, continuing with follow behavior");
    }
    ...
    if (cmd_vel_flag || turn_fin_flag){
        ...
        if (cmd_vel_msg.linear.x == 0.0 && cmd_vel_msg.angular.z == 0.0){
            // A stop command is a one-shot action, not persistent ownership of
            // /cmd_vel. Release the channel after the stop has been delivered.
            cmd_vel_flag = 0;
            turn_fin_flag = 0;
            temp_cout = 0;
            return;
...
rclcpp::Node(node_name)
{
	...
	mapping_motion_blocked_sub = this->create_subscription<std_msgs::msg::Bool>(
		...
		std::bind(&Motion::mapping_motion_blocked_Callback,this,_1));
	/***创建底盘运动话题订阅者***/
...
int main(int argc, char const *argv[])
{
	...
	auto node = std::make_shared<Motion>("motion_control");
	rclcpp::spin(node); 
	rclcpp::shutdown();
	return 0;
