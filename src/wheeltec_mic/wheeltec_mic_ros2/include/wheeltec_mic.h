...
private:
    ...
    int keepalive_interval_ms, serial_retry_interval_ms, serial_open_retries;
    bool process_result;
    bool serial_initialized;
    bool protocol_keepalive;
    bool protocol_ready;
    bool startup_config_sent;
    unsigned char Receive_Data[1024] = {0};
    std::string device_message, usart_port_name, startup_mic_type;
    std::chrono::steady_clock::time_point last_keepalive_tx;
    ...
    bool send_protocol_frame(MsgType type);
    void maybe_send_keepalive();
    void maybe_send_startup_config();
    void publish_voice_flag(int value);
