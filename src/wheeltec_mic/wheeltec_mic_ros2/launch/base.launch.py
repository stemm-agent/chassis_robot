...
def generate_launch_description():
    ...
    voice_recharge_bridge = Node(
        package="wheeltec_mic_ros2",
        executable="voice_recharge_bridge",
        output='screen'
    )

    voice_cartographer_bridge = Node(
        package="wheeltec_mic_ros2",
        executable="voice_cartographer_bridge",
        output='screen'
    )

    nav2_start_feedback_monitor = Node(
        package="wheeltec_mic_ros2",
        executable="nav2_start_feedback_monitor",
        output='screen',
        parameters=[resource_param]
    )
    ...
    ld.add_action(voice_recharge_bridge)
    ld.add_action(voice_cartographer_bridge)
    ld.add_action(nav2_start_feedback_monitor)
