...
namespace
{
constexpr int CHASSIS_SERIAL_IDLE_WAIT_MS = 1;
constexpr float MAX_VALID_LINEAR_SPEED_MPS = 1.5f;
constexpr float MAX_VALID_ANGULAR_SPEED_RADPS = 4.0f;
}
...
bool turn_on_robot::Get_Sensor_Data_New()
{
  ...
  try
  {
    ...
    if (Stm32_Serial.available() < 1)
    {
      std::this_thread::sleep_for(
        std::chrono::milliseconds(CHASSIS_SERIAL_IDLE_WAIT_MS));
      return false;
    }

    size_t bytes_read = Stm32_Serial.read(Receive_Data_Pr,sizeof(Receive_Data_Pr)); //Read the data sent by the lower computer through the serial port //通过串口读取下位机发送过来的数据

    // 超时无数据：立即返回，让 Control() 循环能调用 spin_some() 处理 /cmd_vel 等回调
    // Timeout with no data: return immediately so Control() can call spin_some()
    if (bytes_read == 0)
    {
      return false;
    }
  ...
  catch (const serial::SerialException &e)
  {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
      "Serial read transient failure: %s", e.what());
    return false;
  }
  catch (const serial::IOException &e)
  {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
      "Serial IO transient failure: %s", e.what());
    return false;
  }
    ...
    if(Receive_Data.Frame_Tail == FRAME_TAIL) //Verify the frame tail of the packet //验证数据包的帧尾
    {
      ...
      if(error == 0)
      {
        ...
        */
        ...
        const float measured_x = Odom_Trans(Receive_Data.rx[2],Receive_Data.rx[3]);
        const float measured_y = Odom_Trans(Receive_Data.rx[4],Receive_Data.rx[5]);
        const float measured_z = Odom_Trans(Receive_Data.rx[6],Receive_Data.rx[7]);
        if (!std::isfinite(measured_x) || !std::isfinite(measured_y) ||
            !std::isfinite(measured_z) ||
            std::fabs(measured_x) > MAX_VALID_LINEAR_SPEED_MPS ||
            std::fabs(measured_y) > MAX_VALID_LINEAR_SPEED_MPS ||
            std::fabs(measured_z) > MAX_VALID_ANGULAR_SPEED_RADPS)
        {
          RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
            "Discarding implausible chassis velocity frame: x=%.3f y=%.3f z=%.3f",
            measured_x, measured_y, measured_z);
          return false;
        }

        Receive_Data.Flag_Stop=Receive_Data.rx[1]; //set aside //预留位
        Robot_Vel.X = measured_x; //Get the speed of the moving chassis in the X direction //获取运动底盘X方向速度
        Robot_Vel.Y = measured_y; //The Y speed is only valid on omnidirectional chassis //Y速度仅在全向移动机器人底盘有效
        Robot_Vel.Z = measured_z; //Get the speed of the moving chassis in the Z direction //获取运动底盘Z方向速度
...
turn_on_robot::turn_on_robot():rclcpp::Node ("wheeltec_robot")
{
  ...
  { 
    ...
    serial::Timeout _time = serial::Timeout::simpleTimeout(20); // 20ms timeout keeps callbacks responsive without over-triggering USB serial quirks // 20ms超时，在保证回调及时的同时降低USB串口抖动风险
