...
const double e2 = 1.0 - (EARTH_MINOR * EARTH_MINOR) / (EARTH_MAJOR * EARTH_MAJOR);
...
class GpsTools {
 public:
  ...
  Eigen::Vector3d LLA2ECEF(const Eigen::Vector3d &lla) {
    ...
    double earth_r = EARTH_MAJOR / sqrt(1.0 - e2 * sin(lat) * sin(lat));
    ...
    ecef.z() = (earth_r * (1.0 - e2) + alt) * sin(lat);
